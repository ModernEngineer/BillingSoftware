...
export function SupplierPicker({
  ...
  const filtered = suppliers.filter((s) => {
    ...
    return !q || s.name.toLowerCase().includes(q) || s.mobile.includes(q) || (s.gstin ?? "").toLowerCase().includes(q);
  });
    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search supplier by name, mobile, GSTIN"
        ...
        <div className="absolute z-20 mt-1 max-h-64 w-full overflow-y-auto rounded-md border border-slate-200 bg-white shadow-lg">
          ...
          {filtered.map((s) => (
            ...
            >
              ...
              <span className="ml-2 text-xs text-slate-500">{s.mobile}</span>
            </button>
          ))}
          {filtered.length === 0 && <p className="px-3 py-2 text-sm text-slate-400">No suppliers found.</p>}
        </div>
      )}

      <SupplierFormModal open={addModalOpen} onClose={() => setAddModalOpen(false)} onSave={handleCreate} />
    </div>
  );
}
